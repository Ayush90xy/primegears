...
//   const {setState}=useContext(IdContext);
  ...
  return (
    <TableContainer component={Paper} className="table">
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
          {rows.map((row) => {
            const saleDate = row.soldDate?.seconds
              ? new Date(row.soldDate.seconds * 1000)
              ...
              <TableRow key={row.id}>
                ...
                <TableCell className="tableCell">
                  <Link
                    ...
                    onClick={() => passIT(row.id)}
                    style={{ textDecoration: "none" }}
                  >
                    <div className="tableLink">click to view</div>
                  </Link>
                </TableCell>
              </TableRow>
            );
