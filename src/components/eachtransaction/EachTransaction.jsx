...
const EachTransaction = () => {
  ...
  // ✅ direct id from context
  const { transactionId } = useParams();
  const [mydata, setMyData] = useState(defaultuse);

  useEffect(() => {
    console.log("Effect triggered with id:", transactionId);
    if (!transactionId) return;

    console.log("Subscribing to doc with id:", transactionId);
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setMyData({ ...docSnap.data(), id: docSnap.id });
      } else {
        console.log("No such document for id:", transactionId);
      }
    });
  ...
  const soldDate = mydata.soldDate?.seconds
    ? new Date(mydata.soldDate.seconds * 1000)
    : new Date(mydata.soldDate);

  return (
    <div className="eachtransaction">
      <div className="top">
        <h1>
          Invoice No : <b>{mydata.invoiceNo}</b>
        </h1>
        <h2>
          Customer Name : <b>{mydata.customerName}</b>
        </h2>
        <h2>
          Grand Total : <b>&#8377; {mydata.totalPrice}</b>
        </h2>
        <h3>
          Date Of Sale : <b>{soldDate.toLocaleDateString()}</b>
        </h3>
      </div>

      <div className="middle">
        {mydata.products.map((x) => (
          <div className="item" key={x.productId}>
            <span>
              Product Name : <b>{x.productName}</b>
            </span>
            <span>
              Quantity: <b>{x.quantity}</b>
            </span>
            <span>
              Total Sale: <b>&#8377; {x.total}</b>
            </span>
          </div>
        ))}
      </div>
